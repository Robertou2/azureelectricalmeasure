...
static void SocketEventHandler(EventData *eventData)
{
	...
	//Log_Debug("CANAL RECIBIDO %d", rxBuf[i+1]);
...
static int InitPeripheralsAndHandlers(void)
{
	...
	IN1 = GPIO_OpenAsOutput(AVT_MODULE_GPIO16, GPIO_OutputMode_PushPull, GPIO_Value_Low);
	IN2 = GPIO_OpenAsOutput(AVT_MODULE_GPIO17, GPIO_OutputMode_PushPull, GPIO_Value_Low);
